...
class DatabaseBehatIsolator implements Isolation\IsolatorInterface, EventSubscriberInterface
{
    ...
    {
        $event->writeln('<info>[GorgoDatabaseBehatIsolator] Dumping current application database</info>');
        $this->findCurrentDatabaseEngine()->dump($this->sid, $this->getConfiguration());
        $event->writeln('<info>[GorgoDatabaseBehatIsolator] Dump created</info>');
    }
    ...
    public function afterTest(Isolation\Event\AfterIsolatedTestEvent $event)
    {
        if (1 < $this->featuresCount) {
            $event->writeln('<info>[GorgoDatabaseBehatIsolator] Restoring dump database before next feature</info>');
            $this->findCurrentDatabaseEngine()->restore($this->sid, $this->getConfiguration());
            $event->writeln('<info>[GorgoDatabaseBehatIsolator] finished</info>');
        }
    }
    ...
    {
        $event->writeln('<info>[GorgoDatabaseBehatIsolator] Restoring dump kernel cache</info>');
    ...
    {
        return "GorgoDatabaseBehatIsolator";
    }
